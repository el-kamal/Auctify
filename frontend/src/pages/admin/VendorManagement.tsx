...
export default function VendorManagement() {
    ...
    const fetchVendors = async () => {
        try {
            const data = await ActorsService.getActors("SELLER")
            setVendors(data)
        } catch (error: any) {
            console.error("Failed to fetch vendors", error)
            setError(error.message || "Impossible de charger les vendeurs")
        } finally {
            setLoading(false)
        }
    }

    const handleOpenModal = (vendor?: Actor) => {
        if (vendor) {
            ...
            setFormData({
                ...
                siren_siret: vendor.siren_siret || "",
                address: vendor.address || "",
                iban: vendor.iban || "",
                bic: vendor.bic || "",
                vat_subject: vendor.vat_subject
            })
        } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingVendor) {
                await ActorsService.updateActor(editingVendor.id, formData)
            } else {
    ...
    const filteredVendors = vendors.filter(vendor =>
        ...
        (vendor.siren_siret && vendor.siren_siret.includes(searchTerm))
    )
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {filteredVendors.map((vendor) => (
                            <tr key={vendor.id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">{vendor.phone_number || "-"}</td>
                                <td className="px-6 py-4 whitespace-nowrap">{vendor.siren_siret || "-"}</td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                        ${vendor.vat_subject ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                                        {vendor.vat_subject ? 'Oui' : 'Non'}
                                    </span>
                                </td>
